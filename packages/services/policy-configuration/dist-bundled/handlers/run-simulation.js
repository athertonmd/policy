...
// src/handlers/run-simulation.ts
var run_simulation_exports = {};
__export(run_simulation_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(run_simulation_exports);
...
function runSimulationEngine(input) {
  ...
  const currency = historicalTrips.length > 0 && historicalTrips[0].offers.length > 0 ? historicalTrips[0].offers[0].totalPrice.currency : "GBP";
  return {
    simulationId: `sim_${(0, import_crypto2.randomUUID)()}`,
...
function evaluateTrip(request, graph) {
...
function evaluateByRulePriority(graph, nodeMap, edgesBySource, input) {
...
function evaluateRuleConditions(entryNode, nodeMap, edgesBySource, input) {
  if (entryNode.type === "condition") {
    return entryNode.condition ? evaluateConditionNode(entryNode.condition, input) : false;
  }
...
function evaluateGateCondition(node, nodeMap, edgesBySource, input) {
  const edges = edgesBySource.get(node.nodeId) ?? [];
  if (edges.length === 0) return true;
  const childEdges = edges.filter(
    (e) => e.condition === "default" || e.condition === void 0
  );
  const sortedEdges = [...childEdges].sort(
    (a, b) => (a.priority ?? 100) - (b.priority ?? 100)
  );
  const childTruthValues = [];
  for (const edge of sortedEdges) {
    ...
    if (childNode.type === "condition") {
      const met = childNode.condition ? evaluateConditionNode(childNode.condition, input) : false;
...
function findRuleTerminal(entryNode, nodeMap, edgesBySource, input) {
...
function walkToTerminal(nodeId, nodeMap, edgesBySource, input) {
...
function walkGraph(nodeId, nodeMap, edgesBySource, input) {
  const node = nodeMap.get(nodeId);
  if (!node) {
    return { result: "approve", reasons: [] };
  }
  switch (node.type) {
    ...
    case "condition": {
      const conditionMet = node.condition ? evaluateConditionNode(node.condition, input) : false;
      const edges = edgesBySource.get(nodeId) ?? [];
...
function evaluateConditionNode(condition, input) {
...
function evaluateActionNode(node) {
  if (!node.action) {
    return { result: "approve", reasons: [] };
  }
...
function mergeResults(a, b) {
  ...
  const reasons = [.../* @__PURE__ */ new Set([...a.reasons, ...b.reasons])];
...
function buildChangeReason(activeResult, draftResult) {
  const draftReasons = draftResult.reasons.length > 0 ? `: ${draftResult.reasons.join("; ")}` : "";
...
function roundToFourDecimals(n) {
...
var SIMULATION_BUCKET = process.env.SIMULATION_RESULTS_BUCKET ?? "travel-policy-simulation-results";
var SIMULATION_RETENTION_DAYS = 90;
async function handler(event, _context) {
  ...
  try {
    ...
    let draftGraph;
    try {
      const ast2 = parse(request.draftDslSource);
    ...
    const { activeGraph, historicalTrips } = await withDatabase(async (db) => {
      ...
      if (activeRulesResult.rows.length > 0) {
        ...
        try {
          const ast2 = parse(combinedDsl);
          const result = compile(ast2);
          activeGraph2 = result.policyGraph ?? createEmptyGraph();
        } catch {
          activeGraph2 = createEmptyGraph();
        }
      } else {
        activeGraph2 = createEmptyGraph();
      }
      ...
      const historicalTrips2 = tripsResult.rows.map((row) => {
        try {
      ...
      return { activeGraph: activeGraph2, historicalTrips: historicalTrips2 };
    });
...
async function storeSimulationResult(tenantId, report) {
