...
// src/handlers/rollback-version.ts
var rollback_version_exports = {};
__export(rollback_version_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(rollback_version_exports);
...
// src/rego/rego-compiler.ts
function compileToRego(graph, tenantId, options2 = {}) {
  const { includeComments = true } = options2;
...
function generateRuleModule(graph, ruleMeta, packagePrefix, ruleId, includeComments) {
  const lines = [];
...
function generateMainEvaluator(rules, packagePrefix, includeComments) {
  const lines = [];
  lines.push(`package ${packagePrefix}`);
  lines.push("");
  lines.push("import future.keywords.if");
...
function traverseRuleSubgraph(graph, entryNodeId) {
  const conditions = [];
  let terminal = null;
...
function conditionToRego(condition) {
...
function fieldToRegoPath(field) {
  return `input.${field}`;
}
...
// src/rego/bundle-manager.ts
var BundleManager = class {
  s3;
  eventBridge;
  config;
  constructor(s3, eventBridge, config) {
    this.s3 = s3;
  ...
  async uploadBundle(bundle) {
    const keyPrefix = this.config.keyPrefix ? `${this.config.keyPrefix}/${bundle.tenantId}` : `bundles/${bundle.tenantId}`;
    const bundleKey = `${keyPrefix}/${bundle.bundleId}`;
    for (const module2 of bundle.regoModules) {
      await this.s3.putObject({
        Bucket: this.config.bucketName,
        Key: `${bundleKey}/${module2.path}`,
        Body: module2.content,
   ...
   */
  async publishBundleUpdatedEvent(bundle, uploadResult) {
    const event = {
      version: "1.0",
   ...
   */
  getBundleKey(tenantId, bundleId) {
    const keyPrefix = this.config.keyPrefix ? `${this.config.keyPrefix}/${tenantId}` : `bundles/${tenantId}`;
...
// src/lib/rule-repository.ts
var import_node_crypto = require("node:crypto");
async function getRuleById(db, tenantId, ruleId) {
...
async function getVersion(db, tenantId, ruleId, version) {
  const schemaName = getTenantSchema(tenantId);
  const result = await db.query(
...
async function rollbackToVersion(db, tenantId, ruleId, targetVersion, changedBy) {
  const schemaName = getTenantSchema(tenantId);
...
function mapRowToRule(row) {
  return {
    ...
    policyGraph: JSON.parse(row.policy_graph),
    priority: row.priority,
    status: row.status,
    conditions: JSON.parse(row.conditions),
    outcomes: JSON.parse(row.outcomes),
    version: row.version,
    effectiveFrom: row.effective_from ?? void 0,
...
var BUNDLE_BUCKET = process.env.POLICY_BUNDLE_BUCKET ?? "travel-policy-bundles";
...
async function handler(event, _context) {
  ...
  try {
    ...
    try {
      await eventBridgeClient.send(new import_client_eventbridge.PutEventsCommand({
        Entries: [{
...
async function recompileTenantBundle(tenantId, activeRules) {
...
function createBundleManager() {
  ...
  const eventBridgeAdapter = {
    async putEvents(params) {
      const result = await eventBridgeClient.send(new import_client_eventbridge.PutEventsCommand({
...
//# sourceMappingURL=rollback-version.js.map
