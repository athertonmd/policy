...
// src/handlers/configure-template.ts
var configure_template_exports = {};
__export(configure_template_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(configure_template_exports);
...
async function createTemplate(db, tenantId, templateId, body, createdBy) {
...
async function updateTemplate(db, tenantId, templateId, body) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const result = await db.query(
...
function formatTemplateResponse(record) {
  return {
    ...
    autoApprovalConditions: record.autoApprovalConditionsJson ? JSON.parse(record.autoApprovalConditionsJson) : null,
...
async function handler(event, _context) {
  ...
  try {
    ...
    if (httpMethod === "POST") {
      const templateId = (0, import_node_crypto.randomUUID)();
...
//# sourceMappingURL=configure-template.js.map
