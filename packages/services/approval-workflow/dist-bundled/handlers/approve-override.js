...
// src/handlers/approve-override.ts
var approve_override_exports = {};
__export(approve_override_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(approve_override_exports);
...
async function getOverride(db, tenantId, overrideId) {
...
async function updateOverrideStatus(db, tenantId, overrideId, status, approverId, comment) {
...
async function recordOverrideAudit(db, tenantId, override, approverId, action, comment) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
async function handler(event, _context) {
  ...
  try {
    ...
    const result = await withDatabase(async (db) => {
      ...
      if (!override) {
        return { error: "OVERRIDE_NOT_FOUND" };
      }
      if (override.status !== "pending") {
        return { error: "OVERRIDE_NOT_PENDING", currentStatus: override.status };
      }
      const status = action === "approve" ? "approved" : "rejected";
      const updatedOverride2 = await updateOverrideStatus(
        db,
        tenantId,
        overrideId,
        status,
      ...
      return { override: updatedOverride2 };
    });
    if ("error" in result) {
      switch (result.error) {
        ...
        case "OVERRIDE_NOT_PENDING":
          return errorResponse(
            ...
            `Override is already in '${result.currentStatus}' state`,
            requestId
          );
        case "UPDATE_FAILED":
    ...
    const { override: updatedOverride } = result;
...
//# sourceMappingURL=approve-override.js.map
