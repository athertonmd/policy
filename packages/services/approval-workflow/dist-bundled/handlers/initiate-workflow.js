...
// src/handlers/initiate-workflow.ts
var initiate_workflow_exports = {};
__export(initiate_workflow_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(initiate_workflow_exports);
...
// src/lib/workflow-repository.ts
async function createWorkflow(db, tenantId, workflow) {
...
async function getWorkflowTemplate(db, tenantId, templateId) {
  const result = await db.query(
    `SELECT
  ...
  return {
    ...
    autoApprovalConditions: row.autoApprovalConditionsJson ? JSON.parse(row.autoApprovalConditionsJson) : void 0,
...
function toApprovalWorkflow(record) {
  return {
...
// src/state-machines/single-stage.ts
function generateSingleStageDefinition(params) {
  const {
...
// src/state-machines/multi-stage-sequential.ts
function generateStageStates(stageNumber, slaTimeoutSeconds, notifyApproverLambdaArn, escalationLambdaArn, nextStateName) {
...
function generateMultiStageSequentialDefinition(params) {
  const {
    stages,
...
// src/state-machines/multi-stage-parallel.ts
function generateMultiStageParallelDefinition(params) {
  const {
    approverCount,
    requiredApprovals,
    slaTimeoutSeconds = 86400,
...
// src/state-machines/conditional.ts
function generateBranchStates(branchName, slaTimeoutSeconds, notifyApproverLambdaArn, escalationLambdaArn) {
...
function generateConditionalDefinition(params) {
  const {
    branches,
    defaultBranch,
    defaultSlaTimeoutSeconds = 86400,
  ...
  const allBranchNames = /* @__PURE__ */ new Set([...branches.map((b) => b.name), defaultBranch]);
...
function createStateMachineDefinition(params) {
...
var EVENT_BUS_NAME = process.env.EVENT_BUS_NAME ?? "travel-policy-events";
var NOTIFY_APPROVER_LAMBDA_ARN = process.env.NOTIFY_APPROVER_LAMBDA_ARN ?? "";
var ESCALATION_LAMBDA_ARN = process.env.ESCALATION_LAMBDA_ARN ?? "";
var COMPLETE_WORKFLOW_LAMBDA_ARN = process.env.COMPLETE_WORKFLOW_LAMBDA_ARN ?? "";
async function handler(event, _context) {
  ...
  try {
    ...
    const workflowId = (0, import_node_crypto.randomUUID)();
    const result = await withDatabase(async (db) => {
      ...
      const slaTimeoutSeconds = priority === "urgent" ? template.slaConfig.urgentSlaHours * 3600 : template.slaConfig.defaultSlaHours * 3600;
      ...
      const stages = template.stages.map((stageDef) => ({
        ...
        slaDeadline: new Date(
          Date.now() + stageDef.slaHours * 3600 * 1e3
        ).toISOString()
      }));
      ...
      const workflowRecord2 = await createWorkflow(db, tenantId, {
        workflowId,
        decisionId: body.decisionId,
        tripRequestId: body.tripRequestId,
        travellerId: body.travellerId,
      ...
      return { workflowRecord: workflowRecord2, template };
    });
    if ("error" in result) {
...
function resolveTemplateType(stages) {
  if (stages.length === 1) {
...
//# sourceMappingURL=initiate-workflow.js.map
