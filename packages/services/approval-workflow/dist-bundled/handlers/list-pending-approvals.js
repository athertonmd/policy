...
// src/handlers/list-pending-approvals.ts
var list_pending_approvals_exports = {};
__export(list_pending_approvals_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(list_pending_approvals_exports);
...
// src/handlers/configure-delegation.ts
async function findActiveDelegation(db, tenantId, approverId, atDate) {
  const checkDate = atDate ?? (/* @__PURE__ */ new Date()).toISOString();
...
async function queryPendingWorkflows(db, tenantId, approverId, statusFilter, page, pageSize) {
  const offset = (page - 1) * pageSize;
  const statusCondition = statusFilter ? `AND w.status = $3` : `AND w.status = 'pending'`;
  const params = [approverId, tenantId];
  if (statusFilter) {
...
function extractSlaDeadline(stagesJson, currentStage) {
  try {
    const stages = JSON.parse(stagesJson);
    const stage = stages.find((s) => s.stageNumber === currentStage);
    return stage?.slaDeadline ?? new Date(Date.now() + 24 * 3600 * 1e3).toISOString();
  } catch {
    return new Date(Date.now() + 24 * 3600 * 1e3).toISOString();
  }
}
async function notifyTenantAdminUnavailable(tenantId, workflowId, approverId) {
  try {
...
async function handler(event, _context) {
  ...
  try {
    ...
    const result = await withDatabase(async (db) => {
      const delegationsToApprover = await db.query(
        `SELECT approver_id AS "approverId"
         ...
         WHERE delegate_to_id = $1
           ...
           AND start_date <= $2
           AND end_date >= $2`,
        [approverId, (/* @__PURE__ */ new Date()).toISOString()]
      );
      ...
      const items = ownWorkflows.items.map((row) => ({
        ...
        tripSummary: null,
        // Trip summary would be joined from trip_requests table
        isDelegated: false,
