...
// src/handlers/request-override.ts
var request_override_exports = {};
__export(request_override_exports, {
  REASON_CATEGORIES: () => REASON_CATEGORIES,
  checkOverrideFrequencyLimit: () => checkOverrideFrequencyLimit,
  handler: () => handler
});
module.exports = __toCommonJS(request_override_exports);
...
var MIN_JUSTIFICATION_LENGTH = 10;
var DEFAULT_MAX_OVERRIDES_PER_MONTH = 3;
async function checkOverrideFrequencyLimit(db, tenantId, requestedBy, maxPerMonth) {
...
async function getOverrideFrequencyConfig(db, tenantId) {
  try {
    const result = await db.query(
...
async function createOverrideRecord(db, tenantId, override) {
...
async function handler(event, _context) {
  ...
  try {
    ...
    if (!REASON_CATEGORIES.includes(body.reasonCategory)) {
      return errorResponse(
        400,
    ...
    const overrideId = (0, import_node_crypto.randomUUID)();
    ...
    const result = await withDatabase(async (db) => {
      ...
      const overrideRecord2 = await createOverrideRecord(db, tenantId, {
        overrideId,
      ...
      return { overrideRecord: overrideRecord2 };
    });
    if ("error" in result) {
