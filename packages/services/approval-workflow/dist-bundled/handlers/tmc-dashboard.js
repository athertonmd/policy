...
// src/handlers/tmc-dashboard.ts
var tmc_dashboard_exports = {};
__export(tmc_dashboard_exports, {
  bulkActionHandler: () => bulkActionHandler,
  handler: () => handler,
  workloadHandler: () => workloadHandler
});
module.exports = __toCommonJS(tmc_dashboard_exports);
...
async function queryQueue(db, tenantId, filter, sort, page, pageSize) {
  const { clause: filterClause, params: filterParams } = buildFilterClause(filter);
  const offset = (page - 1) * pageSize;
  const sortField = sort.field === "priority" ? `CASE q.priority WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 WHEN 'normal' THEN 2 ELSE 3 END` : `q.${sort.field === "createdAt" ? "created_at" : sort.field === "slaDeadline" ? "sla_deadline" : "item_type"}`;
  ...
  `;
  const dataResult = await db.query(dataSql, [...filterParams, pageSize, offset]);
...
async function getQueueSummary(db, tenantId) {
  const summarySql = `
    SELECT
...
async function getWorkloadDistribution(db, tenantId) {
  const sql = `
    SELECT
...
async function executeBulkAction(db, tenantId, userId, request) {
  const successful = [];
  const failed = [];
...
async function handler(event, context) {
  ...
  try {
    ...
    const filter = {
      type: params.type ? params.type.split(",") : void 0,
      priority: params.priority ? params.priority.split(",") : void 0,
      status: params.status ? params.status.split(",") : void 0,
      assignedTo: params.assignedTo ?? void 0,
      slaBreached: params.slaBreached ? params.slaBreached === "true" : void 0,
      department: params.department ?? void 0,
...
async function bulkActionHandler(event, context) {
  const requestId = context.awsRequestId;
  try {
    const tenantId = extractTenantId(event);
    if (!tenantId) {
...
async function workloadHandler(event, context) {
  const requestId = context.awsRequestId;
  try {
    const tenantId = extractTenantId(event);
    if (!tenantId) {
...
//# sourceMappingURL=tmc-dashboard.js.map
