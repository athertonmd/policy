...
// src/handlers/submit-action.ts
var submit_action_exports = {};
__export(submit_action_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(submit_action_exports);
...
async function getWorkflow(db, tenantId, workflowId) {
...
async function updateWorkflowStatus(db, tenantId, workflowId, status, currentStage, stagesJson) {
  const completedAt = ["approved", "rejected", "expired", "cancelled"].includes(status) ? (/* @__PURE__ */ new Date()).toISOString() : null;
  let sql = `UPDATE "${tenantId}".approval_workflows
     SET status = $2, completed_at = COALESCE($3, completed_at)`;
  const params = [workflowId, status, completedAt];
  if (currentStage !== void 0) {
    sql += `, current_stage = $${params.length + 1}`;
    params.push(currentStage);
  }
  if (stagesJson !== void 0) {
...
async function recordAction(db, tenantId, action) {
...
async function getTaskToken(db, tenantId, workflowId, stageNumber) {
  const result = await db.query(
...
var POSITIVE_ACTIONS = ["approve"];
var NEGATIVE_ACTIONS = ["reject"];
var ROUTING_ACTIONS = ["delegate", "escalate", "request_info"];
var VALID_ACTIONS = [...POSITIVE_ACTIONS, ...NEGATIVE_ACTIONS, ...ROUTING_ACTIONS];
async function handler(event, _context) {
  ...
  try {
    ...
    if (!body.workflowId || body.stageNumber === void 0 || !body.approverId || !body.action) {
    ...
    const result = await withDatabase(async (db) => {
      ...
      if (workflow.status !== "pending" && workflow.status !== "escalated") {
        return { error: "WORKFLOW_NOT_ACTIONABLE", status: workflow.status };
      }
      const stages = JSON.parse(workflow.stagesJson);
      ...
      const actionId = (0, import_node_crypto.randomUUID)();
      ...
      const updatedStages = stages.map((s) => {
        if (s.stageNumber === stageNumber) {
          return {
            ...s,
            status: action === "approve" ? "approved" : action === "reject" ? "rejected" : s.status,
            approvers: [
              ...s.approvers,
              {
                approverId,
      ...
      const updatedWorkflow2 = await updateWorkflowStatus(
        db,
        tenantId,
        workflowId,
        newStatus,
        action === "approve" && newStatus === "pending" ? stageNumber + 1 : void 0,
        JSON.stringify(updatedStages)
      );
      return { workflow: updatedWorkflow2 };
    });
    if ("error" in result) {
      switch (result.error) {
        ...
        case "WORKFLOW_NOT_ACTIONABLE":
          return errorResponse(
            ...
            `Workflow is in '${result.status}' state and cannot accept actions`,
            requestId
          );
        case "INVALID_STAGE":
    ...
    const { workflow: updatedWorkflow } = result;
    try {
...
//# sourceMappingURL=submit-action.js.map
