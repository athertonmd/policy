...
// src/handlers/auto-approval.ts
var auto_approval_exports = {};
__export(auto_approval_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(auto_approval_exports);
...
async function handler(event, _context) {
  ...
  try {
    const result = await withDatabase(async (db) => {
      ...
      const evaluationResults = conditions.map(
        (condition) => evaluateCondition(condition, tripContext)
      );
...
function evaluateCondition(condition, tripContext) {
...
function resolveFieldValue(fieldPath, context) {
